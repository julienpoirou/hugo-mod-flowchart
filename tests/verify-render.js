...
async function main() {
  ...
  try {
    ...
    await page.waitForSelector('[data-hugo-mod-flowchart][data-rendered="true"]', { timeout: 15000 });

    const svgCount = await page.locator("[data-hugo-mod-flowchart] svg").count();
    ...
    if (svgCount < 1) {
      console.error("FAIL: no <svg> rendered inside the flowchart wrapper");
      process.exit(1);
    }
    ...
    console.log(`PASS: ${svgCount} flowchart(s) rendered to real <svg> (raphael + flowchart.js loaded in order), no errors, no failed requests`);
  } finally {
    await browser.close();
    server.close();
  }
